...
(function($) {
  ...
  Autocomplete.prototype = {
    ...
    initialize: function() {
      ...
      if (!this.update) { 
          this.update = 'Autocomplete_' + uid; 
          $('<div class="autocomplete" id="' + this.update + '" style="display:none; width:"' + this.options.width +'"px;"></div>').appendTo('body');
      }

      this.mainContainerId = this.update;
    ...
    processResponse: function(text) {
      ...
      this.cachedResponse[this.currentValue] = response;
    ...
    select: function(i) {
        if (this.options.selectElement && this.elements.length > 0) {
            this.options.selectElement(this.elements[i]);
            return;
        }
      var selectedValue, f;
    ...
    moveUp: function() {
      ...
      if (this.selectedIndex === 0) {
        $(this.container).find('li').get(0).className = '';
        this.selectedIndex = -1;
        this.el.val(this.currentValue);
        return;
      }
    ...
    moveDown: function() {
      if (this.selectedIndex === ($(this.container).find('li').length - 1)) { return; }
      this.adjustScroll(this.selectedIndex + 1);
    },
