...
// Was 25569, but was calculating dates wrong by 1 day. MZ Thursday, January 11, 2007
...
//		var_dump($res);		
	...
	/**
	 * Convert data for a given sheet to CSV
	 *
	 * @param	int Sheet number to get CSV for
	 * @return	string CSV formatted sheet
	 *
	 * @author	Matt Zabriskie (mattz@emstraffic.com)
	 */
	function toCSV ($sheet = 0)
	{
		$csv = array ();
		for ($i=1; $i<=$this->sheets[$sheet]['numRows']; $i++)
		{
			$row = array ();
			for ($j=1; $j<=$this->sheets[$sheet]['numCols']; $j++)
			{
				// For some reason, if the cell is empty
				//	it doesn't get added to the array.
				if (isset($this->sheets[$sheet]['cells'][$i][$j]))
				{
					$row[] = $this->sheets[$sheet]['cells'][$i][$j];
				} else
				{
					$row[] = "";
				}
	...
	/**
	 * Convert data for a given sheet to Array
	 *
	 * @param	int Sheet number to get Array for
	 * @return	array Array formatted sheet
	 *
	 * @author	Matt Zabriskie (mattz@emstraffic.com)
	 */
	function toArray ($sheet = 0)
	{
		$array = array ();
		for ($i=1; $i<=$this->sheets[$sheet]['numRows']; $i++)
		{
